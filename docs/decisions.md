...
| D75 | 放浪（没带便当）回家的时长**在默认配置里覆盖成 10–20 分钟**，而不是照抄 define.json 的 10/20 当"秒"用 | 用户报告"青蛙出行和回家频率有时会异常，可能两分钟完成一次出门+回家的流程"。定位到 `departFrog`：普通旅行用 `FROG_TRAVEL_MIN/_MAX`（FAITHFUL=1 下 3600–21600 **秒**），放浪分支却把 `FROG_DRIFTRETURNTIME` 的 10/20 当**秒**用，再被 `Math.max(20, window)` 一夹——**每一趟放浪都恰好 20 秒**；加上回家后 60–180 秒的待机间隔，一轮 80–200 秒，正是"两分钟一轮"。用户存档里 `minTripSec: 13` 是这条路径留下的直接证据。触发条件不罕见：`tripPrepared()`（背包里有**任何**东西就算备好）与 `provisionTrip()`（只有**便当**才不算放浪）口径不一致，于是"桌上有工具/护身符、便当已被吃掉"的状态会自我维持，每轮都走放浪分支。改法遵守"`vendor/` 逐字节不动"：引擎本来就支持 `FROG_CONFIG.env` 覆盖这两个值，所以把 600/1200 写进**默认配置**（`src/settings.js`），`/admin` 可改、填 0 或留空即关闭覆盖。为什么按分钟读：这游戏所有旅行时长都在分钟到小时量级，而引擎自己的注释说放浪是一次 "shorter outing"——10–20 分钟正好是"比正常旅行短的一趟"，20 秒则短到玩家一眼看得出。`docs/source-map.md` 的节奏表已同步更正 |
...
| L11 | **「伴蛙同行」（任务面板）的日/周计划永远显示已完成、奖励每次启动都能再领一次** | 2026-09-19 用真浏览器 + 用户真实存档复现：全新启动时「是日清单」3/3、「当周计划」4/5，宝箱每次都在。**两个独立成因，都在引擎内部**（见 D2「不改写引擎」与 L4 同性质）：①`listProgress()` 把每个计划行的进度取自**生涯累计数**——`state.travel.tripCount` 与 `state.pictures.length`——而这两个计数器**从不重置**，可计划名字明明写着是日/当周/半月/月度/当季/年度（`taskData.list_type` 六个类型）；②`task_load_list` 下发的行用 `100*type+i+1` 编号（如 101），客户端却按 `type`（如 1）存取领奖记录（`dataReward`），于是服务端的已领层级永远传不回客户端，每次启动客户端都以为一层都没领过；而服务端对「已领过」和「刚领成功」**都返回 `code: 0`**，客户端分不出来，照样弹奖励框（物品实际没重复发）。**要修必须重新打包引擎**（改 `listProgress` 的周期语义 + 统一领奖记录的键），本项目对 `vendor/` 零修改，所以本版未修 |
| L12 | **「庭院/书桌上的笔记，点带叹号的书会崩溃」未复现，暂按不可复现记录** | 2026-09-19 用 Playwright 驱动真实页面，在四种数据状态下都驱动过：用户自己的存档、用户实际的那 5 条笔记、把 191 个笔记 id 全塞进去、以及只放「附件型」笔记（此时红点显示而 `TravelNoteModel.isOpen()` 为 false，点击会走 `openRoleAttr()` 分支）。**四种全部正常打开笔记界面，零 JS 报错**。代码里确实有无保护的位置（`TravelNoteView.updateRedot` 直接读 `o.config.attach`；`TravelNoteUtils.convertContent2Sources` 遇到缺失的 `info` 会抛），但现有数据路径走不到——引擎只下发客户端 `Note` 表里存在的 id，而 191 行字段齐全。附带澄清一处易错点：**庭院圆桌上的书本美术没有点击区**，真正的点击目标 `enterNoteBtn` 在**室内视图 `MainIn.exml`**（`itemGroup` 内，x=43.65,y=-386.39 的透明矩形），红点 `i_noteRedot` 与之相邻——排查时别在庭院里找。若用户再次遇到，需要的信息是：当时在室内还是庭院、红点出现在书上还是右侧菜单 |
