...
```text
① 直接下包：Releases 页 → frog-nas-1.0.4.fpk → 应用中心「手动安装」
   https://github.com/Kasbuky-sudo/frog-nas/releases/tag/v1.0.4
...
```bash
docker build --platform linux/arm64 -t frog-nas:1.0.4 .
docker buildx build --platform linux/amd64,linux/arm64 -t frog-nas:1.0.4 .
```
