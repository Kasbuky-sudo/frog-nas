...
```bash
bash packaging/fnOS/scripts/verify-fpk.sh dist/frog-nas-1.0.2.fpk 18980
```
...
sleep 8
sudo /usr/local/bin/appcenter-cli install-fpk /vol1/1000/frog-nas-1.0.2.fpk -v 1
sleep 4
...
"$CLI" --profile x86nas --allow-insecure-ws app install-fpk \
    "C:/path/to/dist/frog-nas-1.0.2-cli.fpk" --volume-id 1 --yes
...
# → dist/frog-nas-1.0.2-cli.fpk      （正式包不受影响）
```
